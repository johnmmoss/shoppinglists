...
{
    [HttpPost()]
    public async Task<IActionResult> AddItem([FromRoute] Guid shoppingListId, [FromBody] ShoppingItemAddRequest request)
    {
        var item = new ShoppingItem(shoppingListId, request.CategoryId, request.Name, request.Cost);
        var created = await _shoppingListService.AddItem(item);
